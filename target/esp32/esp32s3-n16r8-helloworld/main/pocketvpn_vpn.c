...
void pocket_vpn_cipher_init(PocketVpnContext *self) {
    ...
    uint32_t occ_size                   = pocketvpn_sprintf((char *)self->client_occ_string, "V4,dev-type tun,link-mtu 1559,tun-mtu 1500,proto TCPv4_CLIENT,cipher %s,auth %s,keysize %d,key-method %d,tls-client", cipher_name, auth_name, (int)self->cipher_key_size * 8, self->key_direction);
...
void pocket_vpn_application_input(PocketVpnContext *self, vBuffer *vbuffer) {
    ...
    if (pocketvpn_memcmp(en_text_ptr, buffer, self->hmac_msg_length) != 0) {
        ...
        pocket_vpn_debug_string(10, "hmac_key_size %d", (int)self->hmac_key_size);
        pocket_vpn_debug_string(10, "hmac_msg_length %d", (int)self->hmac_msg_length);
    ...
    if (en_text_size % self->align_length != 0) {
        pocket_vpn_debug_string(10, "align error! en_text_size: %d (1)", (int)en_text_size);
        return;
    }

    if (en_text_size > sizeof(buffer)) {
        pocket_vpn_debug_string(10, "decrypto error! stack overflow! en_text_size: %d", (int)en_text_size);
        return;
    }
    ...
    if (text_size % self->align_length != 0) {
        pocket_vpn_debug_string(10, "align error! en_text_size: %d (2)", (int)text_size);
        return;
    }

    if (text_size > sizeof(buffer)) {
        pocket_vpn_debug_string(10, "decrypto error! stack overflow! text_size: %d", (int)text_size);
        return;
    }
    ...
    if (padding_len > self->align_length || text_size < padding_len) {
        pocket_vpn_debug_string(10, "padding length error! padding_len: %d, text_size: %d", (int)padding_len, (int)text_size);
        return;
    }
...
void pocket_vpn_check(PocketVpnContext *self) {
    ...
    if (self->socket_write_ready(self->socket_obj) != 0){
        return;
    }
